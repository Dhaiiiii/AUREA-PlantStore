...
// Developed by: Haya (Homepage UI design + product display layout)
// Developed by: Dhay (Database query - fetching products from database)
// Task: Design homepage - Display products from database using SELECT query
session_start();
include 'db.php';
...
if (isset($_COOKIE['past_purchases']) && !empty($_COOKIE['past_purchases'])) {
  $pastIds = array_map('intval', explode(',', $_COOKIE['past_purchases']));
  $idList = implode(',', $pastIds);
  $pastResult = $conn->query("SELECT product_id, name, price, image FROM products WHERE product_id IN ($idList)");
...
<body>
  ...
  <?php if (!empty($pastItems)): ?>
    <!-- Past Purchases Section (Cookie-based) -->
    <section class="section" style="background: rgba(238,246,240,.45);">
      <div class="container">
        <div class="section__head">
          <div>
            <h2>Your Past Purchases</h2>
            <p style="color: var(--muted);">Welcome back! Here are plants you've ordered before.</p>
          </div>
        </div>
        <div class="grid">
          <?php foreach ($pastItems as $row): ?>
  ...
  <footer class="footer">
    <div class="container footer__grid">
      <div><b style="color: var(--ink);">AUREA – Floral & Plants</b></div>
      <div>
