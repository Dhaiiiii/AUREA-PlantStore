...
if (isset($_GET['place_order']) && $_GET['place_order'] == 1) {
  // Update stock in database for each item in cart
  if (!empty($_SESSION['cart'])) {
    foreach ($_SESSION['cart'] as $product_id => $qty) {
      $product_id = (int)$product_id;
      $qty = (int)$qty;
      $conn->query("UPDATE products SET quantity = quantity - $qty WHERE product_id = $product_id AND quantity >= $qty");
    }
  }

  // Save past purchases in cookie (store product IDs for 30 days)
  $pastIds = isset($_COOKIE['past_purchases']) ? $_COOKIE['past_purchases'] : '';
  $newIds = array_keys($_SESSION['cart']);
  $allIds = array_unique(array_merge(
    $pastIds ? explode(',', $pastIds) : [],
    array_map('strval', $newIds)
  ));
  setcookie('past_purchases', implode(',', $allIds), time() + (30 * 24 * 60 * 60), '/');
...
<body>
  ...
  <section class="section">
    <div class="container split" style="grid-template-columns: 1fr .85fr;">
      ...
      <div class="panel">
        ...
        <p style="color: var(--muted); margin-top:6px;">Complete your order details below.</p>
        ...
        <?php else: ?>
          <div style="margin-top:14px; display:grid; gap:10px;">
            ...
            <div style="display:grid; gap:10px;">
              <input id="checkout-name" style="padding:12px 14px; border-radius:14px; border:1px solid rgba(220,234,225,.95); background: rgba(255,255,255,.9)" placeholder="Full name">
              <input id="checkout-phone" style="padding:12px 14px; border-radius:14px; border:1px solid rgba(220,234,225,.95); background: rgba(255,255,255,.9)" placeholder="Phone">
              <input id="checkout-city" style="padding:12px 14px; border-radius:14px; border:1px solid rgba(220,234,225,.95); background: rgba(255,255,255,.9)" placeholder="City, Street">
            </div>
      ...
      <div class="panel">
        ...
        <?php if (empty($cartItems)): ?>
          <div style="margin-top:10px; color: var(--muted); font-weight:650;">No items in cart</div>
        <?php else: ?>
          ...
          <!-- Place Order button with JS validation -->
          <button class="btn btn--primary" style="margin-top:12px; width:100%;" onclick="validateCheckout()">
            Place order
          </button>
  ...
  <script>
    ...
    function validateCheckout() {
      ...
      // All fields filled — proceed to place order
      window.location.href = 'checkout.php?place_order=1';
    }
  </script>
</body>
